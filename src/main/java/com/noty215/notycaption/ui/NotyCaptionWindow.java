...
import com.noty215.notycaption.ai.ProgressWhisper;
import com.noty215.notycaption.ai.WhisperModel;
...
public class NotyCaptionWindow extends JFrame {
    private static final Logger logger = Logger.getLogger(NotyCaptionWindow.class.getName());
    ...
    private ProgressWhisper progressWhisper;
    ...
    private void createMenuBar() {
        ...
        googleLoginItem.addActionListener(e -> onlineHandler.initiateLogin());
    ...
    private void createMainPanel() {
        ...
        loginBtn.addActionListener(e -> onlineHandler.initiateLogin());
        ...
        wordsSpin = new JSpinner(new SpinnerNumberModel(
                ((Number) settings.getOrDefault("words_per_line", 5)).intValue(), 1, 20, 1
        ));
        ...
        // Generate button
        generateBtn = new JButton(translator.tr("generate"));
        generateBtn.setMaximumSize(new Dimension(Integer.MAX_VALUE, 60));
        generateBtn.setBackground(new Color(74, 111, 165));
        generateBtn.setForeground(Color.WHITE);
        generateBtn.addActionListener(e -> startCaptionGeneration());
    ...
    private void createOverlays() {
        ...
        progressContainer.setBorder(BorderFactory.createLineBorder(new Color(74, 111, 165), 2));
        progressContainer.setBorder(BorderFactory.createCompoundBorder(
        ...
        progInfo = new JLabel(translator.tr("starting"));
        ...
        operationProgress.setPreferredSize(new Dimension(400, 30));
        operationProgress.setAlignmentX(Component.CENTER_ALIGNMENT);
        progressContainer.add(operationProgress);

        progressContainer.add(Box.createVerticalStrut(10));

        JPanel speedPanel = new JPanel(new FlowLayout());
        speedPanel.setOpaque(false);
        speedLabel = new JLabel(translator.tr("speed") + " --");
        speedLabel.setForeground(Color.LIGHT_GRAY);
        speedPanel.add(speedLabel);

        etaLabel = new JLabel(translator.tr("eta") + " --");
        ...
        cancelBtn = new JButton(translator.tr("cancel"));
    ...
    private void importMediaFile() {
        ...
        if (fileChooser.showOpenDialog(this) == JFileChooser.APPROVE_OPTION) {
            ...
            // For now, set audioFile to inputFile (in real implementation, would extract audio)
            audioFile = inputFile;
    ...
    private void saveSubtitlesToFile(List<Map<String, Object>> subtitles, String fmt, String path) {
        ...
        try {
            ...
            for (Map<String, Object> sub : subtitles) {
                writer.write(sub.get("index") + "\n");
                writer.write(formatTime((Duration) sub.get("start")) + " --> " + formatTime((Duration) sub.get("end")) + "\n");
                writer.write(sub.get("text") + "\n\n");
            }
            writer.close();
        } catch (Exception e) {
            logger.warning("Failed to save subtitles: " + e.getMessage());
        }
    }
    ...
    private void openModelDownloadDialog() {
        JOptionPane.showMessageDialog(this,
                "Model download feature coming soon.\nPlease download Whisper model manually.",
                "Coming Soon",
                JOptionPane.INFORMATION_MESSAGE);
    }
    ...
    private void proceedToTranscription(String audioToUse) {
        ...
        int wpl = ((Number) wordsSpin.getValue()).intValue();
        ...
        new Thread(() -> {
            try {
                ...
                for (int i = 0; i < 10; i++) {
                    ...
                    sub.put("index", i + 1);
                    sub.put("start", Duration.ofSeconds(i * 5));
                    sub.put("end", Duration.ofSeconds(i * 5 + 4));
        ...
        new Thread(() -> {
            try {
                for (int i = 0; i <= 100; i += 20) {
                    ...
                    final int progress = i;
                    SwingUtilities.invokeLater(() -> updateProgress(progress, "Enhancing..."));
                    Thread.sleep(1000);
                }
                ...
                SwingUtilities.invokeLater(() -> {
                    freezeUI(false, "");
                    JOptionPane.showMessageDialog(this,
                            "Vocal enhancement completed!\nEnhanced file ready for transcription.",
    ...
    private void freezeUI(boolean freeze, String message) {
        ...
        if (freeze) {
            ...
            if (progInfo != null) progInfo.setText(message);
            if (statusValue != null) statusValue.setText(message);
            if (mainProgressBar != null) mainProgressBar.setIndeterminate(true);
        } else {
            overlay.setVisible(false);
            if (statusValue != null) statusValue.setText(translator.tr("idle"));
            if (mainProgressBar != null) {
                mainProgressBar.setIndeterminate(false);
                mainProgressBar.setValue(0);
            }
            if (operationProgress != null) operationProgress.setValue(0);
        }
    }

    private void updateProgress(int value, String message) {
        if (operationProgress != null) operationProgress.setValue(value);
        if (mainProgressBar != null) mainProgressBar.setValue(value);
        if (progInfo != null) progInfo.setText(message);
    }
    ...
    public void updateFromSettings(Map<String, Object> newSettings) {
        this.settings = newSettings;
        if (captionTextArea != null) {
            captionTextArea.setFont(new Font((String) newSettings.getOrDefault("font_family", "Segoe UI"),
                    Font.PLAIN,
                    ((Number) newSettings.getOrDefault("font_size", 12)).intValue()));
        }
        logger.info("Settings updated from dialog");
    }

    public void updateNotebookUrlDisplay(String url) {
        logger.info("Notebook URL: " + url);
    ...
    public void loadDownloadedSubtitles(String filePath) {
        logger.info("Loading downloaded subtitles from: " + filePath);
        // In real implementation, would parse and display subtitles
        JOptionPane.showMessageDialog(this,
    ...
    public void updateOnlineStatusDisplay(String status) {
        if (statusValue != null) {
            statusValue.setText(status);
        }
        logger.info("Online status: " + status);
    }
